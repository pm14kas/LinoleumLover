...
function levelbox:load()
	...
	for kmap, map in pairs(self.game.maps) do
		...
		for kblock, block in pairs(map.blocks) do
			...
			-----------------type conversion-----------------------
			if block.type == "block" then block.type = "Block" end
			if block.type == "spawn" then block.type = "Spawn" end
			if block.type == "hazard" then block.type = "Hazard" end
			if block.type == "target" then block.type = "Portal" end
			if block.type == "checkpoint" then block.type = "Checkpoint" end
			if block.type == "text" then block.type = "Text" end
			if block.type == "AI" then block.type = "AI" end
			...
			if not block.category then block.category = 1 end
			if not block.innerType then block.innerType = 1 end
			-----------------/other-----------------------
...
function levelbox:orderBy(key, array, order)
	...
	local iter = function()   -- iterator function
		...
		else return keys[k], array[keys[k]]
		end
	end
	return iter
end
...
function levelbox:save()
	...
	for kmap, map in pairs(self.game.maps) do
		...
		for kblock, block in pairs(map.blocks) do
			if not save.maps[kmap][block.saveTo] then error("Add arrays for new types!") end
